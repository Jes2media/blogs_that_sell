...
**Skill directory:** `.claude/skills/blog-post-writer/`
...
- **Missing or `setup_complete: false`** → Run setup (read `.claude/skills/blog-post-writer/references/setup/setup-flow.md`)
- **`setup_complete: true`** → Skip to Stage 1
- **User says "reconfigure"** → Re-run setup (read `.claude/skills/blog-post-writer/references/setup/setup-flow.md`)
...
| File | When to Read |
|------|-------------|
| `.claude/skills/blog-post-writer/references/writing-guidelines.md` | Stage 4 — shared quality rules for writer + editor |
| `.claude/skills/blog-post-writer/references/brand-profile.md` | Stage 4 — brand voice profile (created during setup). Overridden if `brand_profile` in `brief.json` points to a custom file. |
| `.claude/skills/blog-post-writer/references/examples/*.md` | Stages 3 & 4 — example articles for structural and voice calibration (if any exist) |
